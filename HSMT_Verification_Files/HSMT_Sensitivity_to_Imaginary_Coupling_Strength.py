#!/usr/bin/env python3
"""
HSMT Phase 3 – Sensitivity to Real Potential Shape (tanh coefficient scaling)
"""
...
class HSMT_Phase3_Real_Potential_Shape_Sensitivity:
    def __init__(self, M=300, delta_rho=0.02, window_size=6, weight_threshold=0.80, tanh_scale=1.0):
        self.M = M
    ...
    def build_master_operator(self):
        ...
        for i in range(N):
            ...
            # Scale the coefficient in front of tanh_u (potential shape)
            V_bi = (np.pi + 0.915965594177219) * 2 * np.pi / 3 * tanh_u / 2.0 * self.tanh_scale
...
# ============================================================
# Run Real Potential Shape Sensitivity
# ============================================================
if __name__ == "__main__":
    print("=" * 82)
    print("HSMT Phase 3 – Sensitivity to Real Potential Shape (tanh coefficient ×0.8, ×1.0, ×1.2)")
    print("Bounded Homogeneity + Full Complex Spectral Region (M=300, window=6, 80% weight)")
    print("=" * 82)

    results = []
    for scale in [0.8, 1.0, 1.2]:
        model = HSMT_Phase3_Real_Potential_Shape_Sensitivity(M=300, delta_rho=0.02,
    ...
    print(f"\n{'tanh ×':>6} | {'Sites':>6} | {'Δρ':>6} | {'H_bounded':>10} | {'C':>7} | {'G_bounded':>10} | {'1/G':>8}")
    print("-" * 72)
    for res in results:
        print(f"{res['tanh_scale']:6.1f} | {res['num_sites']:6d} | {res['Delta_rho']:6.2f} | "
